<template>
    <GModal :show="show" @hide="$emit('hide')" :noHelp=true :showHelp="false" :headless="true">
        <GTable :title="`Jobs for corpus ${corpus.name}`" helpSubject="jobs" :columns
...
watch(jobId, onFinishedJob)
watch(props, onFinishedJob)
